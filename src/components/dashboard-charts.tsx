...
export function DeviceChart({ data }: { data: RealtimeData }) {
  ...
  const total = devices.reduce((a, b) => a + b.users, 0) || 1;

  return (
    <div className="rounded-xl border border-border bg-bg-card p-4 backdrop-blur-sm">
      <h3 className="mb-4 text-sm font-semibold text-text-primary">Users by Device</h3>
          ...
          return (
            <div key={d.device}>
              <div className="mb-1 flex items-center justify-between text-xs">
                <span className="text-text-secondary capitalize">{d.device}</span>
...
export function TopPagesTable({ data }: { data: RealtimeData }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-bg-card p-4 backdrop-blur-sm">
      <h3 className="mb-4 text-sm font-semibold text-text-primary">Top Active Pages</h3>
      {pages.length === 0 ? (
...
export function CountryChart({ data }: { data: RealtimeData }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-bg-card p-4 backdrop-blur-sm">
      <h3 className="mb-4 text-sm font-semibold text-text-primary">Users by Country</h3>
      {countries.length === 0 ? (
        <p className="text-xs text-text-muted">No data available</p>
      ) : (
        <div className="h-52">
